...
if [[ -f "${PLUGIN_DIR}/Jellyfin.Plugin.AkumaGames.dll" ]]; then
  cp -f "${PLUGIN_DIR}/Jellyfin.Plugin.AkumaGames.dll" \
    "${PLUGIN_DIR}/Jellyfin.Plugin.AkumaGames.dll.bak-v0.2.3.5"
fi
...
docker exec "${CONTAINER}" sh -c "
  ...
  cp -f '${WEB_INDEX}' '${WEB_INDEX}.akuma-backup-v0.2.3.5'
...
echo
echo "Akuma Games v0.2.3.5 instalado."
echo "Biblioteca Games e categorias agora usam a navegação nativa instantânea do Jellyfin."
echo "A validação ResolveItem só é executada ao clicar na capa de um game real."
echo "Use Ctrl+Shift+R ou abra uma janela anônima antes de testar."
