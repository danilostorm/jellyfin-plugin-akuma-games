...
if [[ -f "${PLUGIN_DIR}/Jellyfin.Plugin.AkumaGames.dll" ]]; then
  cp -f "${PLUGIN_DIR}/Jellyfin.Plugin.AkumaGames.dll" \
    "${PLUGIN_DIR}/Jellyfin.Plugin.AkumaGames.dll.bak-v0.2.3.6"
fi
...
[[ -s "${SOURCE_DIR}/akuma-games-card-click.js" ]] || { log "bridge rápido não encontrado"; exit 0; }
...
docker exec "${CONTAINER}" sh -c "
  ...
  cp -f '${WEB_INDEX}' '${WEB_INDEX}.akuma-backup-v0.2.3.6'
...
echo
echo "Akuma Games v0.2.3.6 instalado."
echo "A raiz Games agora é desenhada pelo cache rápido do plugin, sem esperar a varredura nativa."
echo "Execute uma sincronização do Akuma Games uma vez para criar folder.jpg nas categorias."
echo "Depois use Ctrl+Shift+R ou abra uma janela anônima antes de testar."
