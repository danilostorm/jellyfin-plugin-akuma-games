...
if [[ -f "${PLUGIN_DIR}/Jellyfin.Plugin.AkumaGames.dll" ]]; then
  cp -f "${PLUGIN_DIR}/Jellyfin.Plugin.AkumaGames.dll" \
    "${PLUGIN_DIR}/Jellyfin.Plugin.AkumaGames.dll.bak-v0.2.3.3"
fi
...
docker exec "${CONTAINER}" sh -c "
  ...
  cp -f '${WEB_INDEX}' '${WEB_INDEX}.akuma-backup-v0.2.3.3'
...
echo
echo "Akuma Games v0.2.3.3 instalado."
echo "Os botões Voltar, Início, menu, busca e usuário foram liberados após fechar o game."
echo "Limpe os dados do site do Jellyfin ou teste em janela anônima."
